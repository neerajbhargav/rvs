      ...
      return (
        <div className="anim-in relative">
          ...
          <div className="relative">
            <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--muted)] pointer-events-none" />
            <input
              type="date"
...
export default function OnboardingPage() {
  ...
  const handleStep1 = async () => {
    ...
    try {
      ...
      await streamPromise; // wait for stream animation to finish
  ...
  return (
    <div className="mx-auto max-w-lg px-6 py-16">
      {step === 4 && <Confetti width={window.innerWidth} height={window.innerHeight} recycle={false} numberOfPieces={500} colors={['#00A19B', '#FF8000', '#EF1A2D', '#001A30']} />}
      
      <div className="mb-12 text-center anim-in">
        <h1 className="text-3xl font-bold display-font mb-2 text-[var(--ink)]">SupportIQ</h1>
        <p className="text-sm text-[var(--muted)]">Configure your workspace</p>
      </div>
      ...
      {/* Dynamic Accent Color based on Step */}
      <div 
        className="glass p-8 anim-in transition-all duration-500" 
        style={{ borderTop: `3px solid ${step === 2 ? 'var(--petronas-teal)' : step === 3 ? 'var(--mclaren-papaya)' : 'transparent'}` }}
      >
        {step === 1 && (
          <div className="space-y-5 anim-fade">
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" className="w-full rounded px-4 py-3 text-sm transition-colors duration-200 bg-[var(--bg)] border border-[var(--border)] focus:border-[var(--ink)] focus:bg-[var(--surface)]" onKeyDown={(e) => e.key === "Enter" && handleStep1()} disabled={loading} />
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider mb-2 text-[var(--ink-soft)] display-font">Password</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full rounded px-4 py-3 text-sm transition-colors duration-200 bg-[var(--bg)] border border-[var(--border)] focus:border-[var(--ink)] focus:bg-[var(--surface)]" onKeyDown={(e) => e.key === "Enter" && handleStep1()} disabled={loading} />
            </div>
            <button onClick={handleStep1} disabled={loading} className="btn-primary w-full mt-8 h-11 flex justify-center items-center">
              {loading ? <div className="h-4 w-4 rounded-full border-2 border-white border-t-transparent animate-spin"></div> : "Continue"}
            </button>
          </div>
        )}
        ...
        {(step === 2 || step === 3) && (
          <div className="anim-fade" key={step}>
            ...
            <div className="flex gap-4 mt-8">
              <button onClick={() => setStep(step - 1)} disabled={loading} className="btn-secondary w-1/3">Back</button>
              <button onClick={() => handleStepSubmit(step + 1)} disabled={loading} className="btn-primary flex-1 flex justify-center items-center">
                {loading ? <div className="h-4 w-4 rounded-full border-2 border-white border-t-transparent animate-spin"></div> : step === 3 ? "Complete" : "Continue"}
              </button>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="text-center anim-fade py-6">
        ...
        {loading && streamText && (
          <div className="mt-6 flex items-center justify-center gap-2 anim-in">
            <div className="w-1.5 h-1.5 rounded-full bg-[var(--petronas-teal)] animate-pulse" />
            <span className="text-xs font-mono font-medium text-[var(--muted)]">{streamText}</span>
          </div>
        )}
      </div>
    </div>
